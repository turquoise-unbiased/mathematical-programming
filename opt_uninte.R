...
p.switch = function(i) {
  switch(i[1],
    rep(ff * sample(ff * 5e2, 1), times = n ^ 2),     # n ^ 3
    rep(ff * sample(ff * 1e3, 1), each  = n, times = n),
    rep(ff * sample(ff * 1e5, 1), each  = n ^ 2))
}

parameters = function() {
  cl = makeForkCluster(min(n * 3, cc))                # cluster of processes
  on.exit(stopCluster(cl))

  P <- array(rep(seq(3), times = n), dim = c(1, 3, n)) # parameters array
...
simulate = function(P) {
  cl = makeForkCluster(min(n ^ 3 * n, cc))            # cluster of processes
  on.exit(stopCluster(cl))
...
if (S.show) { for (i in seq(n)) { print(summary(S[,,i])) ; print(cor(S[,,i])) }}

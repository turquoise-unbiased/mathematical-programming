...
int main() {
  ...
  auto f1 = [=](tick_count t0, tick_count t1) { return  ((t1 - t0).count() / 1e+03); };
  auto println = [=](auto rem, auto score) { std::cout << rem << score << std::endl; };
  ...
  // sum, mean, median, mad
  t[2] = tick_count::now();
  auto sum = std::reduce(par, v.begin(), v.end());
  t[3] = tick_count::now();

  auto mean = sum / static_cast<double>(v.size());

  t[4] = tick_count::now();
  std::sort(par, v.begin(), v.end());
  t[5] = tick_count::now();

  auto median = v.at((v.size() / 2UL));

  t[6] = tick_count::now();
  std::transform(par, v.cbegin(), v.cend(), v.begin(),
                 [=](const auto &elem) { return std::abs(median - elem); });
  t[7] = tick_count::now();
  ...
  println( "1) sum: sum(v)                      ", sum                     );
  println( "2) mean: sum/size(v)                ", mean                    );
  println( "3) median: sort(v)[size(v)/2]       ", median                  );
  println( "4) mad: sort(v-median)[size(v)/2]   ", mad                     );
  // 0 = OK
  return 0;
}
